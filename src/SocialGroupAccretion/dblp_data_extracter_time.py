...
def dblp_data_extracter (file_type, year_r1, year_r2, test_buf, num_of_authors):
    ...
    while 1:
        ...
        if paper_count > 2000000:
            break;
        
        if paper_count /10000 != pointer:
            pointer = paper_count /10000;
            print('Papers counted : ' + str(paper_count));
            last = datetime.datetime.now() - last; 
            print('Time taken since last marked :' + str(last));
        ...
        if str(line).startswith('#@'):
            ...
            if is_abstract == 1 and int(year) >= year_r1 and int(year) <= year_r2 and conference != '' and conf_dict.has_key(conference):
...
dblp_data_extracter ('Test', 1991, 2000, 3, 4)
